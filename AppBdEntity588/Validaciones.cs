...
namespace AppBdEntity588
{
    class Validaciones
    {
        ...
        public bool TipoEmail(string email)
        {
            Regex regla = new Regex(@"^[^@\s]+@[^@\s]+\.[^@\s]+$");

            if (regla.IsMatch(email))
                return true;
            else
            {
                Console.SetCursorPosition(40, 22); Console.Write(" La Entrada debe ser email");
                return false;
            }
        }
